...
INFURA_API_KEY* - used for non-ETX blockchain data (if applicable)
INFURA_API_SECRET* - used for non-ETX blockchain data (if applicable)
...
=======================================================================================
     STEP 8 OF 13. ETX (TRRXITTE) BLOCKCHAIN SUPPORT. (optional)
=======================================================================================

You can set ENABLED_ETX: False or leave it blank to turn it off.

ETX_SAFE_ADDR* - TRRXITTE address. All ETX and TRRXITTE-based token deposits go there
ETX_RPC_URL* - RPC endpoint for TRRXITTE blockchain (default: https://rpc.eth-rpc.trrxitte.com)
...
while true; do
    ...
    if [ "$ENABLED_ETX" = "True" ]; then
        ...
        echo -n "ETX_RPC_URL* (default: https://rpc.eth-rpc.trrxitte.com): "
        read ETX_RPC_URL
        if [ -z "$ETX_RPC_URL" ]; then
            ETX_RPC_URL="https://rpc.eth-rpc.trrxitte.com"
        fi
        export ETX_RPC_URL
    fi
...
Used for sending notifications and alerts. 
You can set IS_SMS_ENABLED: False or leave it blank to turn it off.
...
Used for KYC. 
You can set IS_KYC_ENABLED: False or leave it blank to turn it off.
...
while true; do
    ...
    if [ "$IS_KYC_ENABLED" = "True" ]; then
        ...
        echo -n "SUMSUB_CALLBACK_VALIDATION_SECRET: "
        read SUMSUB_CALLBACK_VALIDATION_SECRET
        export SUMSUB_CALLBACK_VALIDATION_SECRET
    fi
...
Used for KYT. 
You can set IS_KYT_ENABLED: False or leave it blank to turn it off.
...
while true; do
    ...
    if [ "$IS_KYT_ENABLED" = "True" ]; then
        ...
        echo -n "SCORECHAIN_ETX_TOKEN: "
        read SCORECHAIN_ETX_TOKEN
        export SCORECHAIN_ETX_TOKEN
    fi
...
You can set IS_HUMMINGBOT_ENABLED: False or leave it blank to turn it off.
...
services:
    ...
    opencex-etx-blocks:
     container_name: opencex-etx-blocks
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n etx_new_blocks -Q etx_new_blocks -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
    ...
    opencex-deposits:
     ...
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n deposits -Q trx_deposits,bnb_deposits,eth_deposits,matic_deposits,etx_deposits -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
    ...
    opencex-payouts:
     ...
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n payouts -Q trx_payouts,eth_payouts,bnb_payouts,matic_payouts,etx_payouts -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
    ...
    opencex-balances:
     ...
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n check_balances -Q trx_check_balances,bnb_check_balances,eth_check_balances,matic_check_balances,etx_check_balances -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
    ...
    opencex-coin-accumulations:
     ...
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n coin_accumulations -Q trx_accumulations,bnb_accumulations,eth_accumulations,matic_accumulations,etx_accumulations -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
    ...
    opencex-token-accumulations:
     ...
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n tokens_accumulations -Q trx_tokens_accumulations,bnb_tokens_accumulations,eth_tokens_accumulations,matic_tokens_accumulations,etx_tokens_accumulations -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
    ...
    opencex-gas:
     ...
     image: opencex:latest
     command: bash -c "celery -A exchange worker -l info -n send_gas -Q trx_send_gas,bnb_send_gas,eth_send_gas,matic_send_gas,etx_send_gas -c 1 "
     restart: always
     volumes:
      - /app/opencex/backend:/app
     networks:
